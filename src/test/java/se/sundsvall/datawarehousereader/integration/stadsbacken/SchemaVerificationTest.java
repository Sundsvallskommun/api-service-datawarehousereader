...
class SchemaVerificationTest {
	...
	void verifySchemaUpdates() throws IOException, URISyntaxException {
		...
		final var generatedSchema = stripAlterStatements(Files.readString(Path.of(generatedSchemaFile)));

		assertThat(generatedSchema).as(String.format("Please reflect modifications to entities in file: %s", STORED_SCHEMA_FILE))
			.isEqualToNormalizingWhitespace(storedSchema);
	}

	private String stripAlterStatements(final String schema) {
		return schema.replaceAll("(?s)\\s*alter table.*?;", "");
	}
