...
public class InvoiceService {
	...
	private final InvoiceDetailJdbcRepository invoiceDetailJdbcRepository;

	private final InvoiceJdbcRepository invoiceJdbcRepository;

	InvoiceService(final InvoiceRepository invoiceRepository, final InvoiceDetailJdbcRepository invoiceDetailJdbcRepository,
		final InvoiceJdbcRepository invoiceJdbcRepository) {
		this.invoiceRepository = invoiceRepository;
		this.invoiceDetailJdbcRepository = invoiceDetailJdbcRepository;
	...
	public CustomerInvoiceResponse getInvoicesForCustomer(final CustomerInvoiceParameters parameters) {
		...
		invoices.forEach(invoice -> invoice.setDetails(
	 ...
	 * unique across organizations, so the organization number is not needed to identify the details of an invoice and null
	 * is passed for it.
	 * <p>
	 * The assumption was verified directly against Stadsbacken on 2026-08-25: no invoice number in
	 * {@code kundinfo.vInvoiceDetail} (the view that {@code kundinfo.fnInvoiceDetails} replaced) carries rows for more than
	 * one {@code OrganizationId}, and no row has a null {@code OrganizationId}. Should that ever change, this lookup would
	 * attach the details of every organization sharing an invoice number to the same invoice.
	 */
	private Map<Long, List<InvoiceDetail>> fetchDetails(final List<CustomerInvoice> invoices) {
		...
		return invoiceDetailJdbcRepository.getInvoiceDetails(invoiceNumbers).stream()
			.collect(groupingBy(InvoiceDetail::getInvoiceNumber));
	}
	...
	public List<InvoiceDetail> getInvoiceDetails(final String organizationNumber, final long invoiceNumber) {
		return invoiceDetailJdbcRepository.getInvoiceDetails(organizationNumber, invoiceNumber);
	}
}
