...
/**
 * Read district-cooling-measurements tests
 *
 * @see src/test/resources/db/scripts/testdata.sql for data setup.
 */
@WireMockAppTestSuite(files = "classpath:/GetMeasurementsDistrictCooling/", classes = Application.class)
class GetMeasurementsDistrictCoolingIT extends AbstractAppTest {

	private static final String PATH = "/2281/measurements/%s/%s";
	private static final String RESPONSE_FILE = "response.json";

	@Test
	void test01_getDistrictCoolingMonth() {
		setupCall()
			.withServicePath(format(PATH, DISTRICT_COOLING, MONTH) +
	...
	@Test
	void test02_getDistrictCoolingHour() {
		setupCall()
			.withServicePath(format(PATH, DISTRICT_COOLING, HOUR) +
	...
	@Test
	void test03_getDistrictCoolingDay() {
		setupCall()
			.withServicePath(format(PATH, DISTRICT_COOLING, DAY) +
				"?partyId=A0B52C5B-93AC-480B-821D-E238C8F4D95" +
